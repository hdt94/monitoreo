...
class PreProcessChannels(beam.PTransform):
    ...
    def expand(self, channels):
        merged_data = (
            ...
            | beam.Map(self._add_info_to_merged_data)
        )

        return merged_data

    @staticmethod
    def _add_info_to_merged_data(element):
        [key, data_st] = element

        stats = set([(tr.stats.sampling_rate, str(tr.stats.endtime), str(tr.stats.starttime))
                    for tr in data_st])
        assert len(stats) == 1, f"Unexpected multiple stats sets: \"{key}\""
    ...
    def _adjust_data(element):
        ...
        [key, channels] = element
        adjusted_channels = [adjust_channel(ch)
                             for ch in channels]

        return (key, adjusted_channels)
    ...
    def _merge_data_into_obspy_stream(element):
        def get_single_trace(channel):
            ...
            assert len(
                st) == 1, f'Unexpected st with multiple tr for {channel_id}'

            tr = st[0]

            return tr

        [key, data_channels] = element

        # Asume single trace per data obspy stream
        traces = [get_single_trace(ch) for ch in data_channels]
...
class ReadPasscalFiles(beam.PTransform):
    ...
    @staticmethod
    def _read_passcal_into_reftek_obspy_stream(file_metadata):
        reftek_st = obspy_read(FileSystems.open(file_metadata.path))

        return (file_metadata.path, reftek_st)
