...
def start(analysis_config, env_config, input_config, metadata, pipeline_args, save_main_session=False):
    ...
    with beam.Pipeline(options=pipeline_options) as pipe:
        ...
        (
            ...
            | "Responses to rows" >> beam.Map(element_to_row, default_metadata={**metadata, 'type': 'raw'})
        ...
        (raw_responses
            | "Process " >> beam.ParDo(ProcessResponseFn(analysis_config))
            | "process writing" >> beam.Map(reftekio.reftek_obspy_stream_to_response_dict)
            | "Responses rows" >> beam.Map(element_to_row, default_metadata={**metadata, 'type': 'processed'})
